...
public class Gameing_Bg {
    ...
    public Gameing_Bg(Context context, Bitmap bmpbg1, Bitmap bmpbg2, Bitmap bmpbg3, Bitmap bmpcloud_0,
                      Bitmap bmpcloud_1, Bitmap bmpcloud_2, Bitmap bmpcloud_3) {
        this(bmpbg1, bmpbg2, bmpbg3, bmpcloud_0, bmpcloud_1, bmpcloud_2, bmpcloud_3);
        this.context = context;
    }
    private Gameing_Bg(Bitmap bmpbg1, Bitmap bmpbg2, Bitmap bmpbg3, Bitmap bmpcloud_0,
                      Bitmap bmpcloud_1, Bitmap bmpcloud_2, Bitmap bmpcloud_3) {
        this.bmpbg1 = bmpbg1;
        this.bmpbg2 = bmpbg2;
        this.bmpbg3 = bmpbg3;
        ...
        //初始化背景坐标
        bg1X = 0;
        bg1Y = DashSurfaceView.screenH - bmpbg1.getHeight();
        bg2X = 0;
        bg22X = bg2X + bmpbg2.getWidth();
        bg2Y = DashSurfaceView.screenH - bmpbg2.getHeight();
        bg3X = 0;
        bg33X = bg3X + bmpbg3.getWidth();
        bg3Y = DashSurfaceView.screenH - bmpbg3.getHeight();
        //初始化白云坐标
        bc0X = DashSurfaceView.screenW + bmpcloud_0.getWidth();
        bc0Y = (int) (Math.random() * DashSurfaceView.screenH / 3);
        bc1X = bc0X + DashSurfaceView.screenW - bmpcloud_1.getWidth();
        bc1Y = (int) (Math.random() * DashSurfaceView.screenH / 3);
        bc2X = bc0X + DashSurfaceView.screenW - bmpcloud_2.getWidth() + 40;
        bc2Y = (int) (Math.random() * DashSurfaceView.screenH / 3);
        bc3X = bc0X + DashSurfaceView.screenW - bmpcloud_3.getWidth() + 20;
        bc3Y = (int) (Math.random() * DashSurfaceView.screenH / 3);
        //米数坐标
        textX = DashSurfaceView.screenW / 70;
        textY = DashSurfaceView.screenH / 5;
        ...
        meter = 1;
        //实例化背景1的矩形
        rectBg1 = new Rect(bg1X, bg1Y, DashSurfaceView.screenW, DashSurfaceView.screenH);
...
//		canvas.scale(bmpbg1.getWidth()/bg1X, bmpbg1.getHeight()/bg1Y, bmpbg1.getWidth()/2, bmpbg1.getHeight()/2);
        ...
        float textSize = density<1? 18 : 18 * density;
    ...
    public void logic() {
        if (islogic) {
            ...
            if (count % metertime == 0) {
                meter++;
                if (meter == 500)
                    UserAchieveActivity.userAchieve[0] = 1;
                if (meter == 1000)
                    UserAchieveActivity.userAchieve[1] = 1;
                if (meter == 2000)
                    UserAchieveActivity.userAchieve[2] = 1;
                if (meter == 4000)
                    UserAchieveActivity.userAchieve[3] = 1;
            }
            //背景2的速度为基准速度
            bg2X -= bgspeed;
            bg22X -= bgspeed;
            ...
            bg2Y += yy;
            bg3Y += yy;
            if (bg1Y >= DashSurfaceView.screenH - bmpbg1.getHeight() + DashSurfaceView.screenH / 8) {
                yy = -DashSurfaceView.screenH / 40;
            } else if (bg1Y <= DashSurfaceView.screenH - bmpbg1.getHeight()) {
                yy = 0;
                isChange = false;
            }
            ///////////、、、、、、、、、、、、、、、、、、、/////////////////////////
            ...
            if (bc0X <= -bmpcloud_0.getWidth() - 10) {
                ...
                bc2X = bc0X + DashSurfaceView.screenW - bmpcloud_2.getWidth() + (int) (Math.random() * 60) + 10;
                bc2Y = (int) (Math.random() * 110);
            } else if (bc3X <= -bmpcloud_3.getWidth() - 10) {
                bc3X = bc0X + DashSurfaceView.screenW - bmpcloud_3.getWidth() + (int) (Math.random() * 30) + 10;
    ...
    public void onTouchEvent(MotionEvent event, int playercurrentY) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            ///////////、、、、、、、、、、、、、、、、、、、/////////////////////////
            //滑屏加速
            if (x1 - DashSurfaceView.screenW / 20 > x) {
//								Tools.sound_wind = GameMusic.playSound(R.raw.wind, -1);
                ...
                x1 = y1 = 0;
                metertime = 1;
            } else if (x1 < x - DashSurfaceView.screenW / 20) {
//								GameMusic.stopSound(Tools.sound_wind);
    ...
    public int getYy() {
        return bg1Y - (DashSurfaceView.screenH - bmpbg1.getHeight());
    }
