...
public class Pause {
    ...
    public Pause(Context context, Bitmap blackground, Bitmap voice, Bitmap resume, Bitmap quit) {
        this(blackground, voice, resume, quit);
        this.context = context;
    }
    private Pause(Bitmap blackground, Bitmap voice, Bitmap resume, Bitmap quit) {
        this.blackground = blackground;
        this.voice = voice;
        this.resume = resume;
        this.quit = quit;
        this.x = (DashSurfaceView.screenW - blackground.getWidth()) / 2;
        this.y = (DashSurfaceView.screenH - blackground.getHeight()) / 2;
        this.voiceX = DashSurfaceView.screenW / 2 - voice.getWidth() / 4;
        this.voiceY = blackground.getHeight() / 6 * 4;
        this.voiceH = voice.getHeight();
        this.voiceW = voice.getWidth() / 2;
        this.resumeX = DashSurfaceView.screenW / 2 - resume.getWidth() / 4;
        this.resumeY = this.voiceY + voice.getHeight();
        this.resumeH = resume.getHeight();
        this.resumeW = resume.getWidth() / 2;
        this.quitX = DashSurfaceView.screenW / 2 - quit.getWidth() / 4;
        ...
        this.textY = this.y + blackground.getHeight() / 5;
    }

    public void draw(Canvas canvas, Paint paint) {
        float density = Utils.getScreenDensity(context);
        float textSize = density<1? 30 : 30 * density;
...
//					Gameing_Bg.islogic=true;
                ...
                DashSurfaceView.status = Tools.GAME_PLAYING;
//					DashSurfaceView.ispause=false;//将暂停的界面关闭
