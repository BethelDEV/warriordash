...
//		}
		...
		switch(foodMount){
			case 1:
				canvas.drawBitmap(dartShow, 0, DashSurfaceView.screenH/3, paint);
				break;
			case 2:
				canvas.drawBitmap(dartShow, 0, DashSurfaceView.screenH/3, paint);
				canvas.drawBitmap(dartShow, dartShow.getWidth()+5, DashSurfaceView.screenH/3, paint);
				break;
		}
		//受伤时掉血的绘制
		if(isHurt){
	...
	public void ontouch(MotionEvent event,int[] position,int[] position2) {
		...
		if(event.getAction()==MotionEvent.ACTION_UP&&playerStatus!=Tools.PLAYERDART){
			...
			if((event.getX()>=x- DashSurfaceView.screenW/16&&event.getX()<=x+ DashSurfaceView.screenW/16)&&(event.getY()>=y- DashSurfaceView.screenH/32&&event.getY()<=y+ DashSurfaceView.screenH/32)&&!isPaleOnHead){
				jumpTime++;
				if(Tools.isJump){
...
//					jumpTime=1;
			...
			if(event.getX()-x>= DashSurfaceView.screenW/20&&isUpOfPale){
			...
			if(x-event.getX()>= DashSurfaceView.screenW/20&&isUpOfPale){
				setStatus(Tools.PLAYERWALK);
				GameMusic.pauseWind();
			}
		}
	...
	public void eatFood(int kind){
		...
		if(totalFood==10)
			UserAchieveActivity.userAchieve[4]=1;
