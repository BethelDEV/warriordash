...
public class MyButton {
	...
	public MyButton(Bitmap bmpbulletbnt,Bitmap bmppause){
		...
		//初始化按钮坐标
		bulletX= DashSurfaceView.screenW-bmpbulletbnt.getWidth()/2-100;
		bulletY= DashSurfaceView.screenH-bmpbulletbnt.getHeight()-20;
		bulletW=bmpbulletbnt.getWidth()/2;
		bulletH=bmpbulletbnt.getHeight();
		pauseX = 10;
		pauseY= DashSurfaceView.screenH-bmppause.getHeight()-20;
...
//				}
//
//				DashSurfaceView.status=Tools.GAME_PAUSE;
				return true;
			}else if(x>=bulletX&&x<=bulletX+bulletW&&y>=bulletY&&y<=bulletY+bulletH){
				bulletIndex=0;
...
//		if (Tools.GAME_PLAYING!=DashSurfaceView.status) {
//			return;
//		}
		DashSurfaceView.status=Tools.GAME_PAUSE;
